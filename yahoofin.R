...
yfin_archive <- function(path,syms,startDate=format(Sys.time(),DATE_FORMAT),freq='weekly',update=TRUE,.parallel=TRUE){
  archive <- if(file.exists(path)){ get(load(path)) }else{ list() }
